...
public class StudentFee {
    ...
    public static StudentFee create(Long memberId) {
        return new StudentFee(null, memberId, TransferStatus.PENDING, null, null, null);
    }

    // 아직 확인 요청을 한 번도 보내지 않은 회원을 위한 가상의 기본값 (DB에 row가 없음, 저장하지 않는다)
    public static StudentFee notRequested(Long memberId) {
        return new StudentFee(null, memberId, TransferStatus.UNPAID, null, null, null);
    }
    ...
    public StudentFee requestConfirmation() {
        return new StudentFee(id, memberId, TransferStatus.PENDING, null, createdAt, updatedAt);
    }

    public StudentFee review(TransferStatus reviewedStatus, LocalDateTime now) {
