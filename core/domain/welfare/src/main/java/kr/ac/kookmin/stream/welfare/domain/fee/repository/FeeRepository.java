...
public interface FeeRepository {
    Optional<StudentTransferRequest> findByMemberId(Long memberId);
    Optional<StudentTransferRequest> findById(Long id);

    // memberIds가 null이면 전체 대상, 빈 리스트면 결과 없음을 의도한 것이므로 호출 전에 걸러낸다(빈 리스트로 호출하지 않는다)
    PageResult<StudentTransferRequest> search(TransferStatus status, List<Long> memberIds, int page, int size);

    StudentTransferRequest save(StudentTransferRequest request);
}
