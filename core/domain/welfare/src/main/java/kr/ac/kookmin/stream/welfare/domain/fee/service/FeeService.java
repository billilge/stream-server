...
public interface FeeService {
    ...
    StudentTransferStatus getMyFee(Long memberId);

    StudentTransferStatus requestConfirmation(Long memberId);

    // status가 null이면 전체 상태, memberIds가 null이면 회원 필터 없음
    PageResult<StudentTransferStatus> search(String status, List<Long> memberIds, int page, int size);

    StudentTransferStatus review(Long feeId, String status);
